...
class BookService {
    ...
    func downloadBook(id: String) async throws -> URL {
        ...
        print("BookService: URL de téléchargement: \(url.absoluteString)")
        ...
        do {
            print("BookService: Envoi de la requête...")
            let (data, response) = try await URLSession.shared.data(for: request)
            ...
            print("BookService: Réponse reçue - Status: \(httpResponse.statusCode)")
            print("BookService: Headers: \(httpResponse.allHeaderFields)")
            ...
            print("BookService: Taille des données reçues: \(data.count) bytes")
            print("BookService: Enregistrement du fichier à: \(fileURL.path)")
            ...
            print("BookService: Erreur détaillée lors du téléchargement:")
